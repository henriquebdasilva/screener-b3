...
**Guarda de tamanho do e-mail (anti-clipping do Gmail).** O Gmail corta e-mails acima de ~102 KB. O relatório se ajusta automaticamente: se passar de ~96 KB, remove tabelas opcionais em ordem (agenda → preço & risco → indicadores) e, por último, trunca as teses de IA — sempre mantendo tabela principal e preços-teto. Uma nota indica que o detalhe completo está na planilha anexa. A seção Defensivas mostra só a tabela principal (o detalhe desses papéis já aparece em BOVA11/SMALL11). Estilos migrados de inline para classes CSS reduziram bastante o tamanho.
