...
**Legenda da tabela principal + Defensivas completas + nova seção Qualidade+Dividendos.** Três melhorias:
- **Legenda explicativa** sob a tabela de scores (Invest./Qual./Value/Safety/Div./Consist./Critérios), com a definição exata de cada um (conferida contra scoring.py e criteria.py, não inventada).
- **Defensivas agora com todas as sub-tabelas** (Indicadores fundamentalistas, Preço & risco, Estatísticas do ano, Preços-teto, Agenda) — antes só mostrava a tabela de scores com uma nota 'veja BOVA11 acima'.
- **Nova seção 'Qualidade + dividendos'**: papéis com Quality ≥ `--quality-min-div` (default 65) E dividend yield MÉDIO de 5 anos ≥ `--dy-min-div` (default 6.0%), ordenados pelo DY médio (maior primeiro). Usa a coluna `dy_div` (DY médio de 5 anos, com fallback ao DY do último ano quando o histórico de 5 anos não está disponível — deixado explícito na legenda da seção). Mesmas sub-tabelas completas de BOVA11/SMALL11. Testado com dados sintéticos: filtro correto, ordenação correta.

**Qualidade + dividendos agora vem do DIVO11 (índice IDIV), não mais de um corte simples sobre BOVA11/SMALL11.** Mudança de arquitetura, a pedido do usuário:
- **Novo universo DIVO11** (`universe.py`): lista estática de referência (o DIVO11, gerido pela Itaú Asset, não tem CSV de composição automatizável nem como iShares — atualize manualmente, ver aviso no topo do arquivo). Incluído SEMPRE em `get_universe` (independente do `--universe` escolhido), já que alimenta uma seção à parte.
- **Seleção independente**: mesmos cortes fundamentalistas duros e o MESMO cálculo de Investment Score das demais listas (não duplica lógica) — top `--q-divo` (default 0.50 = 50%) por Investment Score DENTRO do próprio DIVO11, sem competir por vaga com BOVA11/SMALL11.
- **Filtro adicional**: `div_ge5_5a` (coluna que já existia, calculada por `paid_dividends_ge` em datafeed.py — reaproveitada, não reimplementada) = pagou DY >= 5%% em TODOS os últimos 5 anos civis completos, não só na média.
- `mailer.py`: `_dividendos_qualidade_section` agora recebe o DataFrame JÁ FILTRADO do screener.py (mesmo padrão de `_defensivas_section`), só formata/exibe.
- Removidos os parâmetros/flags obsoletos `--quality-min-div`/`--dy-min-div` (a filtragem agora é por quantil + o filtro de todos-os-5-anos, não por limiares soltos).
Testado ponta a ponta com dados sintéticos: título, contagem e tabela mostram corretamente os papéis do DIVO11 (diferentes dos de BOVA11/SMALL11).

**DIVO11 agora com lista DINÂMICA (fonte automática real).** Achei a fonte que faltava: a mesma API pública que alimenta a página oficial da B3 (sistemaswebb3-listados.b3.com.br/indexProxy/indexCall/GetPortfolioDay) — sem o bloqueio de JavaScript da página em si, é um endpoint JSON puro, e funciona para QUALQUER índice da B3 (testei contra a composição real do IBOV de hoje, 02/09/2026, 77 ativos, dados batendo). Nova função `_fetch_b3_index_portfolio()` em `universe.py`, usada para o DIVO11 (índice 'IDIV'). `get_universe()` agora tenta a API primeiro e cai na lista estática só se a chamada falhar — mesmo padrão de honestidade/fallback já usado para BOVA11/SMALL11 (iShares). A lista estática continua existindo como rede de segurança, mas deixa de ser a fonte primária. Testado com dados reais da B3 como fixture (parsing) e com mocks (fluxo completo, incluindo fallback quando a API falha).

**PDF sem quebra de página (página contínua).** Trocado `@page{size:A4 landscape}` (altura fixa, ~21cm) por `@page{size:29.7cm 1000cm}` — mesma largura, altura enorme, então o relatório inteiro cabe numa 'folha' só, sem cortar cabeçalho de tabela longe dos valores quando a tabela atravessa o limite de uma página normal. Testado com 15 papéis (conteúdo que antes gerava 8 páginas) -> 1 página só, sem quebra nem espremimento visual.

**Nova seção 'Melhores do AUVP11'.** O AUVP11 replica o Índice Teva Ações Fundamentos (IAFD) — metodologia 100%% fundamentalista (rentabilidade, eficiência operacional, baixa alavancagem), com exclusão setorial de Varejo, Proteína Animal e Transporte Aéreo. Mesmo esquema do DIVO11:
- Novo universo AUVP11 em `universe.py` (lista estática de referência + tentativa de busca dinâmica via a API oficial da B3, código de índice 'IAFD', mesma API já validada para o DIVO11 — cai na lista estática se a API não reconhecer o índice, já que é um índice de terceiros, não necessariamente cadastrado como os índices próprios da B3).
- Seleção independente em `screener.py`: mesmos cortes fundamentalistas duros, top `--q-auvp` (default 0.70 = top 70%%, como pedido) por Investment Score DENTRO do próprio AUVP11 — sem competir por vaga com BOVA11/SMALL11/DIVO11, sem filtro extra (diferente do DIVO11, aqui é só 'melhores ativos', não pagadores de dividendo).
- Nova seção `_auvp_section()` em `mailer.py`, com as mesmas sub-tabelas completas das demais listas.
- `--q-auvp` exposto no disparo manual do `screener.yml` (default 0.70), igual `--q-bluechip`/`--q-smallcap`/`--q-defensive`/`--q-divo`.
Testado ponta a ponta com dados sintéticos: título, contagem, score médio e ordenação corretos.

**AUVP11: fonte real do gestor (BTG Pactual), confirmada pelo usuário.** URL fornecida: `https://www.btgpactual.com/etf/api/Composicao/DownloadCesta?ID=109` — confirmei via fetch que devolve um XLSX de verdade (content-type application/vnd.openxmlformats-officedocument.spreadsheetml.sheet). Nova função `_fetch_btg_etf_xlsx()` em universe.py (usa openpyxl, já é dependência do projeto). Cascata de fallback GENERALIZADA: cada item do plano de universo agora aceita uma LISTA de fontes tentadas em ordem — para o AUVP11: BTG (XLSX do gestor, mais confiável p/ ETFs de terceiros) -> API B3 (código IAFD, pode não estar cadastrado) -> lista estática. Testado: quando o BTG funciona, a API B3 nem é chamada (evita trabalho desnecessário); quando o BTG falha, cai corretamente na API B3.

**IMPORTANTE — parser ainda não validado contra o arquivo real.** O parser do XLSX é GENÉRICO por enquanto (varre todas as células procurando algo que pareça um ticker B3) — funciona na teoria, mas não foi testado contra a estrutura de colunas real do arquivo do BTG (nome da coluna de ticker, linhas de cabeçalho/rodapé, etc.). Pedido ao usuário para baixar e enviar o arquivo, para eu validar/ajustar o parser com precisão contra os bytes reais — ainda pendente na próxima interação.

**Parser do BTG validado contra a planilha REAL (não é mais genérico).** O usuário mandou o XLSX de verdade (Cesta_20260902 — uma cesta de integralização real do AUVP11, 33 ativos). Estrutura confirmada: aba 'CestaIntegralizacao', 4 linhas em branco, cabeçalho 'TICKER '|'ATIVO '|'QUANTIDADE '|'PREÇO DE ABERTURA (R$) '|'PESO (%) ' (com espaços à direita nos nomes), dados logo abaixo. Reescrito `_fetch_btg_etf_xlsx()` para localizar a coluna 'TICKER' pelo cabeçalho (em vez de assumir posição fixa — arquivos de datas diferentes podem ter blocos de cabeçalho de tamanho diferente) e ler só essa coluna.

**Bug real encontrado e corrigido: B3SA3 (ticker da própria B3) não batia com o regex.** `_TICKER_RE` exigia 4 LETRAS seguidas de 1-2 dígitos — mas o ticker da B3 é 'B3SA3' (o '3' faz parte da raiz, não só do sufixo de classe). Ajustado para aceitar dígito em qualquer posição da raiz de 4 caracteres, mantendo a 1ª posição como letra. Testado: os 33 tickers da planilha real batem exatamente (incluindo B3SA3), e os testes anteriores (parser da API B3, lista estática) continuam passando sem regressão.

**Tendência de curto prazo nas seções de Opções e Humor do mercado.** A pedido do usuário: as duas seções agora mostram a evolução dos últimos dias, não só a fotografia do dia — usando o mesmo cache de histórico já existente (`atualizar_historico_bdi`, generalizado para aceitar campos extras via `**extras` em vez de parâmetros fixos, e agora mescla campos em vez de sobrescrever quando chamado mais de uma vez no mesmo dia).
- **Opções — rankings do dia**: novo mini-gráfico 'Tendência — P/C por volume (mercado)' — mostra se o mercado está ficando mais defensivo (P/C subindo) ou mais comprado (P/C caindo) nas últimas sessões, com cor mudando automaticamente (verde ≤0,8, vermelho ≥1,2).
- **Humor do mercado**: novo mini-gráfico 'Tendência — % dos papéis em alta (MM21)' para BOVA11 e SMALL11 separadamente — mostra se a amplitude do mercado está melhorando ou piorando nos últimos dias.
Testado ponta a ponta com dados sintéticos simulando uma escalada de P/C (0,70 -> 1,20) e queda de amplitude (55%% -> 35%%): os dois gráficos renderizam corretamente, com as cores e a tendência visualmente óbvias.

**Histórico e Fluxo estrangeiro: 'conta pra trás' quando o BDI do dia não avança.** A pedido do usuário, depois de identificar (via log real) que dias consecutivos caindo no mesmo `data_ref` (BDI incompleto, fallback repetido) travavam o cálculo do delta e mostravam 'n/d':
- `atualizar_historico_bdi()`: se a data de referência de hoje NÃO for mais recente que a última já salva no histórico, a função agora NÃO sobrescreve nada — mantém o histórico exatamente como estava, contando a partir do último dia que teve dado novo de verdade. Testado: dia sem avanço real preserva os dados existentes; avanço genuíno em seguida volta a funcionar normalmente.
- Painel macro e Resumo de mercado: quando o fluxo do dia não está disponível, agora buscam no histórico o ÚLTIMO dia que teve um valor real e mostram esse valor claramente rotulado com a data dele (ex.: '+1.307 R$ mi (último dado disp., 28/08/2026)') em vez de só '(dia disponível amanhã)' — não finge que é de hoje, só não fica mudo quando já temos um dado bom guardado.
- Também ajustado o horário do cron (screener.yml) de 19:17 para 22:30 BRT, dando mais de 3h extras pra B3 publicar o BDI completo antes da execução automática (não afeta disparos manuais).

**Busca retroativa: cobertura por campo, não só o total de dias (resolve P/C volume ficar preso em 2 dias).** A `preencher_historico_retroativo()` só rodava se o TOTAL de dias no histórico fosse menor que a janela — mas fluxo e OI já tinham 15/15 dias, então a função nunca mais rodava, mesmo com o P/C por volume (usado no gráfico 'Tendência' da seção de opções) tendo só 2 dias (só os que acumularam naturalmente dia a dia desde que essa métrica foi adicionada). Agora a função checa a cobertura de CADA campo separadamente (fluxo/OI/P-C-volume) e só busca o(s) que realmente estão incompletos — evita chamadas de rede desnecessárias nos campos já cobertos. Novo bloco de busca retroativa do P/C por volume via COTAHIST (arquivo diário da B3, mesmo que o opcoes.py já usa no dia a dia — `_baixar_cotahist`/`parse_cotahist` reaproveitados, não duplicados). Testado: só busca o campo faltante (não refaz fluxo/OI já cobertos); backfill do P/C volume preenche corretamente quando os dados estão disponíveis.

**CAUSA RAIZ do fluxo estrangeiro 'n/d' encontrada e corrigida — a seção do BDI vem ATRASADA 2-4 pregões.** Com os 3 PDFs reais que o usuário mandou (BDI_02 de 31/08, 01/09 e 02/09), descobri algo que muda tudo: a seção 'Participação dos Investidores' do BDI **não é do dia do arquivo** — ela vem com atraso. Confirmado nos 3 arquivos reais:
- BDI 'de 31/08' → dados 'do início do mês até o dia **27/08**'
- BDI 'de 01/09' → dados 'até o dia **28/08**'
- BDI 'de 02/09' → dados 'até o dia **31/08**'
O código usava a DATA DO ARQUIVO como se fosse a data dos dados de fluxo — errado. Isso explicava tudo: os deltas pareciam 'travados' porque comparávamos dias que na prática apontavam pra dados de dias BEM anteriores e às vezes coincidentes. `parse_fluxo_acumulado()` já extraía a data real ('até o dia DD/MM/AAAA') via regex, mas SÓ no modo texto corrido (fallback) — o modo por coordenadas, que roda sempre primeiro e quase sempre funciona, tinha `data_base` HARDCODED como `None`. Corrigido: agora extrai a data real sempre, não importa qual modo pegou os números. Testado ponta a ponta com os 3 arquivos reais em sequência (simulando 3 execuções diárias): os deltas agora saem corretos e plausíveis (-R$86mi em 28/08, +R$458mi em 31/08) — antes davam 'n/d' nos dois.

**Histórico: chave volta a ser o dia do RELATÓRIO, não a data atrasada do fluxo — resolve breadth/P-C-volume/OI ficarem presos.** A correção anterior (usar a data real extraída do texto, atrasada 2-4 pregões) resolveu o cálculo do delta do fluxo, mas criou um problema novo: como TODOS os campos (fluxo, OI, P/C volume, amplitude do mercado) eram gravados sob essa MESMA data atrasada, campos que são frescos todo dia (OI/P-C-volume/breadth) ficavam presos na data atrasada do fluxo em vez de avançar normalmente. Corrigido: `screener.py` agora chama `atualizar_historico_bdi(..., data_ref=dt.date.today())` — a chave do histórico volta a ser 'o dia em que o relatório foi gerado', não a data que cada dado individualmente descreve (essa continua aparecendo separada, ao lado do valor, no Panorama macro). Mesma correção em `preencher_historico_retroativo`: a busca retroativa do fluxo agora guarda (data_real, saldo) internamente pra calcular os deltas certos, mas grava o resultado sob a data do ARQUIVO. Testado ponta a ponta com os 3 PDFs reais mapeados pra 3 dias de calendário consecutivos: as chaves do histórico ficam nos dias certos (dia do relatório) e os deltas batem exatamente com o cálculo manual (-R$86mi, +R$458mi).

**Filtro de sanidade contra delta implausível do fluxo (achado direto no historico_bdi.json real do usuário).** O arquivo real mostrou um 'fluxo_dia' de +R$18.927mi em 03/09 — um salto absurdo (todo o resto do histórico fica entre ~50 e ~2.600 em módulo). Hipótese mais provável: a seção 'Participação dos Investidores' do BDI é ocasionalmente REVISADA pela B3 depois de publicada (o mesmo 'até o dia X' pode ter valores diferentes em publicações diferentes) — comparar uma leitura antiga com uma revisada gera um delta sem sentido. Em vez de tentar adivinhar exatamente quando isso acontece, adicionado um LIMITE DE SANIDADE (±R$8.000mi, bem generoso frente ao range real observado): delta fora disso é descartado (vira None/indisponível) em vez de publicado — nunca inventa/aceita um número implausível. Aplicado nos dois caminhos (cálculo diário e busca retroativa). Testado: delta absurdo (+18.927) é corretamente descartado; delta plausível (+2.000) continua passando normalmente.

**Cobertura do fluxo: corrigido para checar 'fluxo_dia', não 'fluxo_acum_mes'.** A busca retroativa considerava o fluxo 'coberto' só por ter o acumulado presente — mas o que importa pro gráfico é o 'dia' (delta), que pode faltar mesmo com o acumulado presente. Corrigido para checar a cobertura do campo certo.

**Amplitude do mercado (breadth) — limitação real, não corrigida.** Diferente de fluxo/OI/P-C-volume, a amplitude (% de papéis em alta) não tem um arquivo único da B3 pra baixar retroativamente — vem de reprocessar o universo inteiro de ações. Backfill retroativo NÃO implementado por não ser viável com esforço razoável; a métrica continua acumulando naturalmente a partir de quando foi introduzida (02/09 em diante).

**Autolimpeza do outlier já gravado no cache real do usuário.** O filtro de sanidade da rodada anterior só impedia NOVOS deltas implausíveis de entrar — o +R$18.927mi que já estava gravado no `historico_bdi.json` real do usuário continuaria lá indefinidamente. Adicionada autolimpeza: toda vez que o histórico é lido (tanto na atualização diária quanto na busca retroativa), qualquer `fluxo_dia` que já viole o limite de sanidade é removido — só esse campo específico, o resto do dia (acumulado, OI, breadth) continua intacto. Constante `LIMITE_FLUXO_DIA` unificada (antes duplicada em dois lugares). Testado contra uma reconstrução do histórico real do usuário: o outlier de 03/09 some sozinho na próxima leitura, sem afetar o resto do dado daquele dia.

**Quatro novas métricas por ativo: Momentum, Value at Risk, Índice de Sharpe e Desvio Padrão.** Adicionadas à tabela 'Estatísticas do ano' de todas as seções (BOVA11, SMALL11, Defensivas, Qualidade+dividendos, AUVP11):
- **Desvio padrão (dia)**: o desvio-padrão diário 'cru' dos retornos (~1 ano), sem anualizar — complementa a Volatilidade (que já é esse número × √252).
- **Momentum (12-1)**: retorno dos últimos 12 meses EXCLUINDO o último mês — fator clássico de momentum (Jegadeesh & Titman), evita capturar reversão de curtíssimo prazo.
- **VaR 95% (1d)**: Value at Risk histórico (percentil 5 dos retornos diários reais — não assume distribuição normal, mais robusto a caudas gordas que o VaR paramétrico).
- **Sharpe**: (retorno anualizado − Selic) ÷ volatilidade anualizada — usa a Selic já disponível no pipeline como taxa livre de risco.
Implementadas em `datafeed.py` (`price_stats`, que agora recebe `selic` como parâmetro). Testado: valores plausíveis contra série sintética com drift/volatilidade conhecidos, e o desvio padrão diário bate matematicamente com Volatilidade÷√252 (confirma consistência interna). Renderização real conferida em PDF — 15 colunas na tabela, ainda legível graças à página contínua (sem quebra).

**Consistência agora pune lucro/patrimônio encolhendo (a pedido do usuário).** Descoberta: o critério 'ROE crescente' e um critério de lucro (comparação simples 1º vs. último ano) já existiam na Consistência — que JÁ pesa 15%% na nota final (Investment Score), não é só exibida. Faltava patrimônio líquido. Mudanças em `criteria.py`:
- **Novo critério `patrimonio_cresc_5a`**: usa `trend_up()` (mesmo método robusto de EBITDA/margem/ROE/ROIC) sobre o patrimônio líquido por ano (já buscado antes, só não usado como critério — reaproveitado de `equity_by_year` do balanço).
- **`cresc_lucro_5a` upgradado**: antes comparava só o 1º e o último ano (podia ser enganado por um ano bom isolado); agora usa `trend_up()` quando o lucro por ano está alinhado (maioria dos anos em alta, não só as pontas) — cai pro método simples se não tiver o alinhamento por ano.
`screener.py`: monta o lucro por ano alinhado (mesmo padrão já usado pro ROE por ano) e passa os dois novos parâmetros pro `consistency()`. Legenda da tabela principal atualizada. Testado: empresa com patrimônio/lucro crescendo tira nota de consistência 100; encolhendo tira 78 — diferença real de ~3 pontos no Investment Score final com o peso padrão de 15%%.

**As 7 melhorias do prompt de modelo probabilístico, implementadas e conectadas às tabelas do relatório.** A pedido do usuário, inspiradas num prompt de valuation single-stock — adaptadas em versão 'leve' e escalável para os ~150+ papéis do universo:
1. **CAGR em janelas decrescentes** (5a/3a/1a, receita) + flag de desaceleração — `cagr_janelas()` em datafeed.py, reaproveita a receita por ano (já buscada internamente pro cálculo de margem, só não era exposta — agora é).
2. **Crescimento implícito no preço** (reverse DCF leve) — inverte a mesma fórmula do DCF já usada nos preços-teto (`compute_ceilings`), sem nenhum cálculo novo de verdade. Validado matematicamente (aplicar de volta reproduz o preço exato).
3. **Faixa de preço probabilística 1 ano** (P10/P50/P90) — projeção analítica lognormal a partir do retorno médio e volatilidade dos retornos LOG diários (~1 ano); sem Monte Carlo, é conta fechada.
4. **Dispersão entre métodos de teto** — já existia calculada (`teto_dispersao`, `teto_confiavel`), só nunca tinha sido exibida; agora aparece como alerta ⚠ na tabela de preços-teto.
5. **Liquidez média diária (R$)** — volume financeiro médio dos últimos ~21 pregões; nova coluna em Preço & risco.
6. **Gatilho de falseamento na tese de IA** — prompt do Gemini atualizado pra pedir explicitamente um evento observável e quantificado que provaria a análise errada; `_fmt_metrics` agora também alimenta a IA com CAGR por janela, desaceleração, crescimento implícito e anos atípicos.
7. **Sinalização de ano atípico** — `ano_atipico()` em datafeed.py, usa mediana/MAD (mais robusto que média/desvio-padrão, que se deixam 'puxar' pelo próprio outlier em amostras pequenas — descoberto e corrigido durante os testes).
Todas testadas isoladamente e depois com o PDF completo renderizado. Novo bloco de alerta (fundo amarelo) na tabela de Indicadores fundamentalistas, só aparece quando há algo pra sinalizar.
