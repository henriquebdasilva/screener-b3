...
**Cortes duros de margem líquida e ROE.** Por padrão, o screener agora reprova (corte duro, antes do percentil): não-financeiras com **margem líquida < 8%** (`--min-margin`, 0 desliga; não se aplica a bancos/seguros) e qualquer papel com **ROE < 10%** (`--min-roe`, 0 desliga; vale para todos os setores). Dado ausente não reprova sozinho. Colunas `margem_ok`/`roe_ok` na planilha.
